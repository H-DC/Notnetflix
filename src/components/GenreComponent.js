...
function GenreComponent({
    ...
    let calcScrollWidth = (numOfMovies) => {
        ...
        let titleMapLen = isBigPoster
            ? Math.ceil((vwUnitToEndBig + 92) / 100)
            : Math.ceil((vwUnitToEnd + 92) / 100);
    ...
    let rightArrowClass = isBigPoster
        ? 'arrow rightArrow bigArrow'
        : 'arrow rightArrow normalArrow';
    let leftArrowClass = isBigPoster
        ? 'arrow leftArrow bigArrow'
        : 'arrow leftArrow normalArrow';
        ...
        <div className="genreContainer">
            <div className="titleContainer">
                <h2 className="genreTitle">
                    {genre}
                    <MdChevronRight className="genreTitleChevron" />
                </h2>
                ...
                {moviesList.map((e, i) => (
                    <div
                        className={
                            isBigPoster
                                ? 'movieCard'
                                : 'movieCard movieCardLabel'
                        }>
                        <img
                            className={posterClass}
                            src={
                                isBigPoster
                                    ? tmdbUrl + e.poster_path
                                    : tmdbUrl + e.backdrop_path
                            }
                            alt={e.name}
                            onMouseEnter={(evt) =>
                                handleHover({ isBigPoster, movieData: e, evt })
                            }
                            onMouseLeave={() => handleLeavePoster()}
                        />
                        {!isBigPoster ? (
                            <div className="posterLabel">
                                <span className="posterLabelSpan">
                                    {(
                                        e.name ||
                                        e.original_name ||
                                        e.original_title ||
